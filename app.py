...
def load_artifacts():
    # load the LSTM directly from HDF5
    from tensorflow.keras.models import load_model
    model     = load_model("dual_lstm_model.h5")
    scaler_Xr = pickle.load(open("scaler_X_raw.pkl","rb"))
    scaler_Xe = pickle.load(open("scaler_X_eng.pkl","rb"))
    scaler_y  = pickle.load(open("scaler_y.pkl","rb"))
    df_feat   = pickle.load(open("df_feature_history.pkl","rb"))
    up_th,down_th = pickle.load(open("trend_thresholds.pkl","rb"))
